...
public class GEGrammar extends ContextFreeGrammar implements ParameterI {
    ...
    protected GEChromosome genotype;
    Phenotype phenotype;
    protected int maxWraps;
    protected int usedWraps;
    protected int usedCodons;
    String name;
    protected DerivationTree dT;
    protected int maxDerivationTreeDepth;
    protected int maxCurrentTreeDepth;
    protected int dTNodeCount;
    ...
    public boolean genotype2Phenotype(boolean tt) {
    ...
    public Rule getRule(Symbol nT_symbol){
        if(nT_symbol.getType()!=Enums.SymbolType.NTSymbol){
            return null;
        }
        for(int i=0;i<rules.size();i++){
            if(rules.get(i).getLHS().equals(nT_symbol)){
                return rules.get(i);
            }
        }
        return null;
    ...
    protected int getNextNT_Index(LinkedList<Symbol> phentype, int index){
        for(int i=index; i<phentype.size();i++){
            if(phentype.get(i).getType()==Enums.SymbolType.NTSymbol)
                return i;
        }
        return -1;
    }
     ...
     * depth of the tree. 
     * (length = Sum_i=0^D a^genUsed, D=max depth, a = max non terminals in a production)
     * 
